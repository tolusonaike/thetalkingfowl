...
class FoundationNavWalker extends Walker_Nav_Menu {
  ...
  public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
    ...
    if ( $depth === 0 ) {
      $output .= '<li' . $id . $value . $class_names . '>' . (!empty($item->title) ? '<a href="' . $item->url . '">' . $item->title . '</a>' : $item->title );
    }
    ...
    if ( $depth === 1) {
      ...
      if (!empty($item->url)) {
        $output .= '<a href="' . $item->url . '" class="deep-1">' . $item->title . '</a>';
      }
    ...
    if ( $depth > 1) {
      ...
      if (!empty($item->url)) {
          $output .= '<a href="' . $item->url . '" class="deep-2">' . $item->title . '</a>';
      }
